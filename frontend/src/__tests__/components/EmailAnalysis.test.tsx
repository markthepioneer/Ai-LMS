...
describe('EmailAnalysis Component', () => {
  ...
  it('handles email analysis successfully', async () => {
    ...
    await waitFor(() => {
      ...
      // Check sentiment and priority using partial text match
      expect(results).toHaveTextContent('Sentiment:');
      expect(results).toHaveTextContent(mockResults.sentiment);
      expect(results).toHaveTextContent('Priority:');
      expect(results).toHaveTextContent(mockResults.priority);
